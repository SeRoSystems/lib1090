...
/**
 * Decoder for track and turn (BDS 5,0), ICAO Doc 9871 (First Edition, AN/464)
 * §A.2 TABLE A-2-80, p. 91
 */
...
public class TrackAndTurn extends BDSRegister implements Serializable {
    ...
    static boolean extractRollAngleSign(byte[] message) {
        return ((message[0] >>> 6) & 0x1) == 1;
    }

    static short extractRollAngleValue(byte[] message) {
        return (short) ((((message[0] & 0x03F) << 3) | ((message[1] >>> 5) & 0x07)) & 0x1FF);
    }

    static boolean extractTrueTrackAngleStatus(byte[] message) {
        return ((message[1] >>> 4) & 0x1) == 1;
    }

    static boolean extractTrueTrackAngleSign(byte[] message) {
        return ((message[1] >>> 3) & 0x1) == 1;
    }

    static short extractTrueTrackAngleValue(byte[] message) {
        return (short) ((((message[1] & 0x007) << 7) | ((message[2] >>> 1) & 0x07F)) & 0x3FF);
    }

    static boolean extractGroundSpeedStatus(byte[] message) {
        return (message[2] & 0x01) == 1;
    }

    static short extractGroundSpeedValue(byte[] message) {
        return (short) ((((message[3] & 0x0FF) << 2) | ((message[4] >>> 6) & 0x03)) & 0x3FF);
    }

    static boolean extractTrackAngleRateStatus(byte[] message) {
        return ((message[4] >>> 5) & 0x1) == 1;
    }

    static boolean extractTrackAngleRateSign(byte[] message) {
        return ((message[4] >>> 4) & 0x1) == 1;
    }

    static short extractTrackAngleRateValue(byte[] message) {
        return (short) ((((message[4] & 0x00F) << 5) | ((message[5] >>> 3) & 0x01F)) & 0x1FF);
    }

    static boolean extractTrueAirSpeedStatus(byte[] message) {
        return ((message[5] >>> 2) & 0x01) == 1;
    }

    static short extractTrueAirSpeedValue(byte[] message) {
